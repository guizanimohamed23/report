...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Use case diagram placeholder for Sprint 2}}%
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{11cm}|}
        ...
        \textbf{Description} & The user provides a target (URL) and optional parameters; the backend validates input, creates a `Scan` job, enqueues it and returns a job identifier. \\ \hline
        \textbf{Pre-condition} & The user is authorized to submit scans and the queue service is available. \\ \hline
        \textbf{Post-condition} & A `Scan` job is stored and queued for processing; the user receives the job id. \\ \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{11cm}|}
        ...
        \textbf{Description} & The worker parses raw tool outputs, maps fields to the unified `ScanResult` schema, and records structured findings. \\ \hline
        \textbf{Pre-condition} & Raw output produced by a scanner is available on disk or in storage. \\ \hline
        \textbf{Post-condition} & A normalized `ScanResult` object is produced and passed to the matcher and persistence layers. \\ \hline
    \end{tabular}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Class diagram placeholder for scanning and matching services}}%
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Placeholder: Scan Workflow Diagram (submission → queue → worker → normalizer → matcher → DB)}}%
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Sequence diagram placeholder for scan processing pipeline}}%
...
This sprint is implemented on the existing Node.js backend and the React frontend. The backend adds the `scanController` and `scanService` modules, normalization and matcher submodules, and worker/scheduler code. The frontend adds or adapts `ScannerPage.tsx`, `ScanHistoryPage.tsx` and a `ScanDetailView` component.

On the backend, key endpoints include `POST /api/scan` for submission, `GET /api/scan/:id` for status, and `GET /api/scan/:id/results` for results. Worker processes consume jobs from the queue, run configured scanners, and store raw outputs for normalization.

On the frontend, the `ScannerPage` allows scan submission and shows job ids; the `ScanHistoryPage` lists past scans with filters; the `ScanDetailView` displays matched vulnerabilities and normalized findings.

The following figures are reserved for screenshots and visual aids:
