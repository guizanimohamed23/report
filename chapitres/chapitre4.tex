...
This sprint is implemented on the existing Node.js backend and the React frontend. The backend adds the \texttt{scanController} and \texttt{scanService} modules, normalization and matcher submodules, and worker/scheduler code. The frontend adds or adapts \texttt{ScannerPage.tsx}, \texttt{ScanHistoryPage.tsx} and a \texttt{ScanDetailView} component.

On the backend, key endpoints include \texttt{POST /api/scan} for submission, \texttt{GET /api/scan/:id} for status, and \texttt{GET /api/scan/:id/results} for results. Worker processes consume jobs from the queue, run configured scanners, and store raw outputs for normalization.

On the frontend, the \texttt{ScannerPage} allows scan submission and shows job ids; the \texttt{ScanHistoryPage} lists past scans with filters; the \texttt{ScanDetailView} displays matched vulnerabilities and normalized findings.
