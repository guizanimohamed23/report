...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Sequence diagram placeholder for login and protected access}}%
    \caption{Sequence diagram for authentication and access control}
    \label{fig:sprint3-sequence-diagram}
\end{figure}
...
On the backend, the main authentication endpoints are defined under \texttt{/api/auth}. The register and login endpoints create secured sessions, the guest endpoint creates a limited temporary session, and the \texttt{/me} endpoint returns the current authenticated user. The backend stores users in PostgreSQL through Prisma and uses bcrypt to hash passwords before saving them.
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Screenshot placeholder: scanner page}}%
    \caption{Scanner page}
    \label{fig:scanner-page}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:scan-history-page}
\end{figure}

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering Screenshot placeholder: scan details page}}%
    \caption{Scan details page}
    \label{fig:scan-details-page}
\end{figure}
