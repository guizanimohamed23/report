...
\indent\justify This chapter presents the analysis and specification of the \textbf{CyberScope} system. It introduces the adopted development methodology, identifies the main stakeholders, presents the development environment, and defines both functional and non-functional requirements. It also presents the use case diagram, the textual description of the main use cases, and the organization of the project through the product backlog and sprint planning.

\section{Adopted Development Methodology}

\indent\justify To ensure a flexible and progressive development process, the Scrum methodology was adopted for the realization of \textbf{CyberScope}. Scrum is an agile framework based on iterative development, incremental delivery, continuous feedback, and adaptation to evolving requirements.

\subsection{Justification of the Choice of Scrum}

\indent\justify The choice of Scrum is justified by the nature of the CyberScope project, which involves several heterogeneous components such as a frontend dashboard, a backend API, a Python-based scanning engine, external security tools, and AI-based analysis modules.

\indent A traditional sequential development approach would make it difficult to validate each component progressively. Scrum allows the project to be divided into smaller increments, making it possible to test, validate, and improve each part of the system throughout development.

\subsection{Adaptation of Scrum to an Individual Project}

\indent\justify Although Scrum is typically applied in team-based environments, it was adapted to the context of an individual end-of-studies project. The student assumed multiple roles simultaneously, including development, planning, and coordination.

\indent The academic and professional supervisors contributed as stakeholders by validating the objectives, monitoring progress, and providing feedback. This adaptation preserved the benefits of Scrum while accommodating the constraints of a single-developer project.
\noindent Figure~\ref{fig:scrum-cyberscope} illustrates the Scrum development process adapted to the CyberScope project, showing the iterative sprint cycles, ceremonies, and feedback loops that guided the development of all three project sprints.
...
\indent CyberScope is organized into three iterative sprints, each with distinct objectives and deliverables.     Each sprint delivers a cohesive set of user stories that align with its main objective. 
 \indent Table~\ref{tab:sprint-organization} presents the sprint timeline and main goals, showing how the project progresses from backend infrastructure through tool integration to advanced attack graph generation over a 12-week development cycle.
\indent Table~\ref{tab:product-backlog} consolidates all seventeen user stories across the three sprints, with each story categorized by type, associated priority level, and sprint assignment. This backlog provides a comprehensive view of the system's functional requirements and guides development efforts from inception through delivery.
\begin{table}[H]
    \centering
...
\indent\justify For faster, higher-level estimates during roadmap planning we applied the T-shirt sizing method (XS, S, M, L, XL). This method groups user stories by relative size classes rather than numeric points, allowing quick comparison of complexity and effort across items. The T-shirt sizing scale we used is illustrated in Figure~\ref{fig:tshirt-sizing}.
...
\indent\justify The product backlog consolidates all identified user stories, features, and technical tasks prioritized for implementation. Table~\ref{tab:product-backlog} lists the backlog items with their assigned sprint, category, and priority, providing a roadmap for development and release planning.
...
\indent\justify The functional requirements define the core services provided by CyberScope. These functionalities describe how the system interacts with users and how it performs automated security analysis, from scan initiation to result reporting.
...
\indent The use case diagram illustrates the main interactions between the user and the CyberScope system.
\noindent Figure~\ref{fig:global-usecase} illustrates the main actors (User, Guest User, and System) and their interactions with the CyberScope platform, showing the primary use case flows for authentication, scanning, and attack scenario generation.
...
\indent Development work was performed on a Windows workstation using the following primary tools:

\vspace{0.3cm}

\noindent Figure~\ref{fig:vscode-logo} shows Visual Studio Code, the primary development environment used for writing frontend and backend code, providing advanced features for debugging and extension management.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Lightweight, extensible editor used for coding, debugging and workspace management.
    \end{minipage}
\end{figure}
\noindent Figure~\ref{fig:git-logo} represents Git, the distributed version control system used for managing source code, tracking development history, and coordinating collaborative contributions throughout the project lifecycle.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Source control system used to track changes and manage project history.
    \end{minipage}
\end{figure}

\noindent Figure~\ref{fig:chromium-logo} shows the Chromium/Google Chrome browser, used for manual functional testing, dynamic content verification, and network analysis of the CyberScope frontend during development.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Browser used for manual testing, dynamic debugging and DOM/network inspection.
    \end{minipage}
\end{figure}

\noindent Figure~\ref{fig:drawio-logo} represents diagrams.net (draw.io), the diagramming tool used to create all architectural diagrams, use case diagrams, class diagrams, and sequence diagrams throughout the documentation.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Diagramming tool used to design system architecture and UML diagrams.
    \end{minipage}
\end{figure}

\noindent Figure~\ref{fig:overleaf-logo} shows Overleaf, the online collaborative LaTeX editor used for writing, editing, and compiling the project documentation and technical report.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Online LaTeX editor used for collaborative editing and compiling documentation.
    \end{minipage}
\end{figure}
...
\noindent Figure~\ref{fig:node-logo} represents Node.js, the JavaScript runtime environment providing the execution platform for the CyberScope backend server and all related processes.

\vspace{0.3cm}
\noindent Figure~\ref{fig:express-logo} shows Express.js, the lightweight web framework used to define all RESTful API endpoints, routing logic, and middleware components in the CyberScope backend.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize JavaScript runtime used as the backend execution environment.
    \end{minipage}
\end{figure}

\vspace{0.3cm}

\noindent Figure~\ref{fig:prisma-logo} represents Prisma, the Object-Relational Mapping tool providing type-safe database access, migrations, and schema management for the PostgreSQL database.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Minimal web framework used to build the RESTful API and routing logic.
    \end{minipage}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize ORM used to manage database schema and interact with PostgreSQL.
    \end{minipage}
\end{figure}

\vspace{0.3cm}

\noindent Figure~\ref{fig:postgresql-logo} shows PostgreSQL, the robust relational database management system used to persistently store user data, scan jobs, vulnerabilities, and analysis results.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Relational database used to persist scans, users and results.
    \end{minipage}
\end{figure}
...
\noindent
\textbf{Frontend}
\noindent Figure~\ref{fig:react-logo} represents React, the JavaScript library used to build the interactive CyberScope frontend dashboard with reusable components, state management, and dynamic user interfaces.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Library used to build the interactive dashboard and UI components.
    \end{minipage}
\end{figure}
...
\noindent Figure~\ref{fig:vite-logo} shows Vite, the modern frontend build tool and development server used to provide fast hot module replacement and optimized production builds for the React frontend.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Development server and build tool providing fast HMR.
    \end{minipage}
\end{figure}

\vspace{0.3cm}

\noindent Figure~\ref{fig:typescript-logo} represents TypeScript, the typed superset of JavaScript used throughout both frontend and backend development to provide static type checking and improved code quality.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Typed superset of JavaScript used to improve code quality and catch errors early.
    \end{minipage}
\end{figure}
...
\hspace{10cm}
\break
\noindent Figure~\ref{fig:python-logo} shows Python, the programming language used to develop the scanning orchestration engine, tool integration scripts, and auxiliary processing utilities in CyberScope.

\vspace{0.2cm}
\begin{figure}[H]
    \centering
    \includegraphics[width=2.2cm]{uploads/logos/python.png}
    \caption{Python}
    \label{fig:python-logo}
    \vspace{0.2cm}
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Used for the scanning engine and glue code to integrate external tools.
    \end{minipage}
\end{figure}

\noindent Figure~\ref{fig:ollama-logo} shows Ollama, the local AI engine used to assist in analysis and prompt generation within CyberScope.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Local AI engine used to assist in analysis and prompt generation.
    \end{minipage}
\end{figure}
\noindent Figure~\ref{fig:hexstrike-logo} shows HexStrike AI, the orchestration layer for automated execution of multiple security tools in CyberScope.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.9\textwidth}
        \centering
        \footnotesize Orchestration layer for automated execution of multiple security tools.
    \end{minipage}
\end{figure}
