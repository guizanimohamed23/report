...
\indent Development work was performed on a Windows workstation using the following primary tools; Figure~\ref{fig:vscode-logo} shows Visual Studio Code logo, the primary development environment used for writing frontend and backend code, providing advanced features for debugging and extension management.
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
\indent Figure~\ref{fig:git-logo} represents Git logo , the distributed version control system used for managing source code, tracking development history, and coordinating collaborative contributions throughout the project lifecycle.
...
\indent Figure~\ref{fig:chromium-logo} shows the Chromium/Google Chrome browser logo, used for manual functional testing, dynamic content verification, and network analysis of the CyberScope frontend during development.
...
\indent Figure~\ref{fig:drawio-logo} represents diagrams.net (draw.io) logo, the diagramming tool used to create all architectural diagrams, use case diagrams, class diagrams, and sequence diagrams throughout the documentation.
...
\indent Figure~\ref{fig:overleaf-logo} shows Overleaf logo , the online collaborative LaTeX editor used for writing, editing, and compiling the project documentation and technical report.
...
\indent Figure~\ref{fig:node-logo} represents Node.js logo, the JavaScript runtime environment providing the execution platform for the CyberScope backend server and all related processes.
...
\indent Figure~\ref{fig:express-logo} shows Express.js logo, the lightweight web framework used to define all RESTful API endpoints, routing logic, and middleware components in the CyberScope backend.
...
\vspace{0.3cm}
\indent Figure~\ref{fig:prisma-logo} shows Prisma logo, the Object-Relational Mapping tool providing type-safe database access, migrations, and schema management for the PostgreSQL database.
...
\indent Figure~\ref{fig:postgresql-logo} shows PostgreSQL logo, the robust relational database management system used to persistently store user data, scan jobs, vulnerabilities, and analysis results.
...
\indent Figure~\ref{fig:react-logo} represents React logo, the JavaScript library used to build the interactive CyberScope frontend dashboard with reusable components, state management, and dynamic user interfaces.
...
\indent Figure~\ref{fig:vite-logo} shows Vite logo, the modern frontend build tool and development server used to provide fast hot module replacement and optimized production builds for the React frontend.
...
\indent Figure~\ref{fig:typescript-logo} represents TypeScript logo, the typed superset of JavaScript used throughout both frontend and backend development to provide static type checking and improved code quality.
...
\indent Figure~\ref{fig:ollama-logo} shows Ollama logo, the local AI engine used to assist in analysis and prompt generation within CyberScope.
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
\indent Figure~\ref{fig:hexstrike-logo} shows HexStrike AI logo, the orchestration layer for automated execution of multiple security tools in CyberScope.
