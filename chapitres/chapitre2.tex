...
The development of \textbf{CyberScope} was carried out on a Windows-based workstation. The main tools used in this environment were \textbf{Visual Studio Code}, which served as the primary code editor for development, debugging, and workspace management; \textbf{Git}, which was used to track changes and manage the source code history; and \textbf{Google Chrome / Chromium}, which was used for manual testing, dynamic debugging, and inspecting web application behavior.

\subsection{Software Environment}

The software environment of \textbf{CyberScope} is organized around three main layers: the backend API, the frontend interface, and the external intelligence and scanning tools. On the backend, the system relies on \textbf{Node.js} as the runtime environment, \textbf{Express.js} for the RESTful API and routing logic, \textbf{Prisma} as the ORM used to interact with the PostgreSQL database and manage schema migrations, and \textbf{PostgreSQL} for persisting scan data, users, and results. On the frontend, \textbf{React} is used to build the interactive dashboard and components, while \textbf{Vite} provides the development server and fast hot-module reloading, and \textbf{TypeScript} improves code quality and reduces runtime errors.

The project also integrates external tools and services for scanning, orchestration, and AI-assisted analysis. \textbf{Python} is used to implement the scanning engine and the glue code for tool integrations, \textbf{Ollama} serves as the local AI engine for generating analysis prompts and assisting risk assessment, and \textbf{HexStrike AI} acts as the orchestration layer and automation platform for managing multiple security tools. In addition, \textbf{Chromium / Google Chrome} is used as the browser runtime for dynamic crawlers and headless scanning components.
